...
class ODPersistentVolumeClaim():
    ...
    async def delete_pv( self, name:str):
        """delete_pv
        ...
        try:
            ...
            await self.kubeapi.delete_persistent_volume( name=name, async_req=True )
        except ApiException as e:
            self.logger.error( e )
        

    async def delete_pvc( self, authinfo:AuthInfo, userinfo:AuthUser )->V1PersistentVolumeClaim:
        self.logger.debug('')
        assert_type( authinfo, AuthInfo)
        assert_type( userinfo, AuthUser)
        ...
        if oc.od.settings.desktop['removepersistentvolumeclaim'] is True: 
            # look for pvc
            pvc = await self.find_pvc( authinfo=authinfo, userinfo=userinfo )
            ...
            try:
                ...
                deleted_pvc = await self.kubeapi.delete_namespaced_persistent_volume_claim( 
                ...
                if isinstance( deleted_pvc , V1PersistentVolumeClaim):
                    ...
                    if isinstance( volume_name, str ):
                        if oc.od.settings.desktop['removepersistentvolume'] is True:
                            ...
                            await self.delete_pv( volume_name )
            except ApiException as e:
                self.logger.error( e )
        
        return deleted_pvc

    async def create( self, authinfo:AuthInfo, userinfo:AuthUser, persistentvolume_request:dict, persistentvolumeclaim_request:dict )->V1PersistentVolumeClaim:
        self.logger.debug('')
        assert_type( authinfo, AuthInfo)
        assert_type( userinfo, AuthUser)

        # look for pvc
        pvc = await self.find_pvc( authinfo=authinfo, userinfo=userinfo, persistentvolumeclaim=persistentvolumeclaim_request )
        ...
        if isinstance( persistentvolume_request, dict ): 
            ...
            persistentvolume  = await self.create_pv( authinfo=authinfo, userinfo=userinfo, persistentvolume_request=persistentvolume_request, suffix=suffix )
    ...
    async def waitforBoundPVC( self, name:str, queue: asyncio.Queue=None )->tuple:
        ...
        while continue_reading_events:
            try:
                async for event in w.stream(  self.kubeapi.list_namespaced_persistent_volume_claim, 
                                        namespace=self.namespace, 
    ...
    '''
    async def waitforBoundPVC( self, name:str, callback_notify, timeout:int=42 )->tuple:
        self.logger.debug('')
        assert_type( name, str)
        pvc = None
        c = 0
        while( c<timeout):
            c = c + 1
            pvc = await self.kubeapi.read_namespaced_persistent_volume_claim(name=name, namespace=self.namespace)
            if isinstance(pvc, V1PersistentVolumeClaim) :
                if isinstance( pvc.status, V1PersistentVolumeClaimStatus):
                    # A volume will be in one of the following phases:
                    #   Available -- a free resource that is not yet bound to a claim
    ...
    async def unrelease_pv( self, persistentvolume:V1PersistentVolume )->V1PersistentVolume:
        ...
        newpv = None
        try: 
            newpv = await self.kubeapi.patch_persistent_volume( name=persistentvolume.metadata.name, body=pv_patch )
    ...
    async def find_pv( self, authinfo:AuthInfo, userinfo:AuthUser, persistentvolume:V1PersistentVolume )->V1PersistentVolume:
        self.logger.debug('')
        ...
        if isinstance( pv_name, str ):
            ...
            pv = await self.get_persistent_volume( name=pv_name )
        ...
        pv_list = await self.kubeapi.list_persistent_volume( label_selector=label_selector )
    ...
    async def find_pvc( self, authinfo:AuthInfo, userinfo:AuthUser, persistentvolumeclaim:dict=None )->V1PersistentVolumeClaim:
        self.logger.debug('')
        ...
        if isinstance( pvc_name, str ):
            ...
            pvc = await self.get_persistent_volume_claim( name=pvc_name )
            if isinstance( pvc, V1PersistentVolumeClaim):
                return pvc

        label_selector= self.get_label_selector( self.get_labels(authinfo, userinfo) ) 
        self.logger.debug( f"look for pvc label_selector={label_selector}" )
        pvc_list = await self.kubeapi.list_namespaced_persistent_volume_claim( namespace=self.namespace, label_selector=label_selector )
    ...
    async def create_pv( self, authinfo:AuthInfo, userinfo:AuthUser, persistentvolume_request:dict, suffix:str=None )->V1PersistentVolume:
        self.logger.debug('')
        assert_type(authinfo, AuthInfo)
        assert_type(userinfo, AuthUser)
        ...
        pv = await self.find_pv( authinfo, userinfo, persistentvolume=body )
        if isinstance( pv, V1PersistentVolume ):
            ...
            if isinstance( pv.status, V1PersistentVolumeStatus):
                if pv.status.phase == 'Released':
                    ...
                    unreleased_pv = await self.unrelease_pv( pv )
                    if isinstance( unreleased_pv, V1PersistentVolume):
                        pv = unreleased_pv
            return pv 
        
        pv = await self.kubeapi.create_persistent_volume( body=body )
    ...
    async def create_pvc( self, authinfo:AuthInfo, userinfo:AuthUser, persistentvolume:V1PersistentVolume, persistentvolumeclaim:dict, suffix:str=None )->V1PersistentVolumeClaim:
        self.logger.debug('')
        assert_type(authinfo, AuthInfo)
        assert_type(userinfo, AuthUser)
        ...
        pvc = await self.find_pvc( authinfo, userinfo, persistentvolumeclaim=body )
        if isinstance( pvc, V1PersistentVolumeClaim ):
        ...
        pvc = await self.kubeapi.create_namespaced_persistent_volume_claim( namespace=self.namespace, body=body )
        return pvc

    async def get_persistent_volume_claim( self, name:str )->V1PersistentVolumeClaim:
        pvc = None
        assert_type( name, str)
        try:
            pvc = await self.kubeapi.read_namespaced_persistent_volume_claim( namespace=self.namespace, name=name )
        except ApiException as e:
            pass
        return pvc
    
    async def get_persistent_volume( self, name:str )->V1PersistentVolume:
        pv = None
        assert_type( name, str)
        try:
            pv = await self.kubeapi.read_persistent_volume( name=name )
        except ApiException as e:
            pass
        return pv
